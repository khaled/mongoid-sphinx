module MongoidSphinx
  class Index
    ...
    def initialize(model)
      ...
      @model        = model
      @options      = model.sphinx_index_options
    ...
    def options
      all_sphinx_index_options = config.sphinx_index_options.clone
      @options.keys.select { |key|
        MongoidSphinx::Configuration::IndexOptions.include?(key.to_s)
      }.each { |key| all_sphinx_index_options[key.to_sym] = @options[key] }
      all_sphinx_index_options
    end
    ...
    def set_configuration_options_for_indexes(index)
      config.sphinx_index_options.each do |key, value|
        method = "#{key}=".to_sym
